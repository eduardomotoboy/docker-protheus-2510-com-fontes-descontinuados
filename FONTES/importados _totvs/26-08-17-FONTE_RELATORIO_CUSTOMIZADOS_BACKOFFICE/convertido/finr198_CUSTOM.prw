...
Static Function ReportPrint(oReport)
	...
	bVlOrig := {|| (cTMPSE5198)->E5_ULTBX == "S" .And. U_F198VldC(cTMPSE5198) }
	bVlBxd  := {|| U_F198VldC(cTMPSE5198) }
		...
		While (cTMPSE5198)->E5_NATPAI == cNatPai .And. !(cTMPSE5198)->(Eof())
			If cCodNat != (cTMPSE5198)->E5_NATUREZ
				If (cTMPSED198)->(DBSeek(cCodNat))
					...
					oSecTot:PrintLine()
					oSecTot:Finish()
					U_F198IncC(oReport, cCodNat, "aTotais" )
					oSecTit:Init()
				EndIf
			EndIf
		...
		While cNatPai <> ""
			If (cTMPSED198)->(dbSeek(cNatPai))
				If 	(cTMPSED198)->NIVEL == 1 //Só imprime o totalizador da sintética no último nível
					...
					oSecTot:PrintLine()
					oSecTot:Finish()
					U_F198IncC( oReport, (cTMPSED198)->NATUREZA, "aTotSint")
					oReport:IncMeter()
				Else
	...
	*****************************************/
	...
	If (lCreate := (MsCreate(cTMPSE5198, aStruct, "TOPCONN")))
		...
		While (cAliasQry)->(!Eof())
			...
			aValores := U_F198To1C(cAliasQry, cCarteira, @oRatSev, @oQryFk1, @oQryFk2, lAchouSE1, lAchouSE2)
...
User Function F198TotC()
	...
	U_F198To2C() //Monta os totalizadores por natureza sintética
Return
