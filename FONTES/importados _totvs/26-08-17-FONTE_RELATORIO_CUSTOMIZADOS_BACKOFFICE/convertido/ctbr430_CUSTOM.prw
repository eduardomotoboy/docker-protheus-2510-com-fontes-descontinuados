...
If lImpLivro
	...
	MsgMeter({|	oMeter, oText, oDlg, lEnd | ;
				U_CtbGerRC(oMeter,oText,oDlg,lEnd,@cArqTmp,cEntGerIni,cEntGerFim,cMoeda,dDataIni,dDataFim,;
...
Do While lImpLivro .And. !cArqTmp->( Eof() )
	...
	// Conta Sintetica	
	cContaSint := U_Ctr430SC(cArqTmp->CONTA,@cDescSint,cMoeda,@cDescConta,cCodPlGer)
...
U_CtbRazGC(oMeter,oText,oDlg,lEnd,cEntGerIni,cEntGerFim,cMoeda,dDataIni,dDataFim,;  
		...
		While !Eof() .and. CT2->CT2_FILIAL == cFilCT2 .And. ;
			...
			If lIsRedStor
				If __lMovEnt05
					...
					EndIf
				Endif
				U_CtbGrvRC(cMoeda,tpSaldo,"1",cContaG,cNormal,CTS->CTS_IDENT)
			Else		
				U_CtbGrvRC(cMoeda,tpSaldo,"1",cContaG,cNormal)
			EndIF
			lNoMovDeb := .F.
		...
		While !Eof() .and. CT2->CT2_FILIAL == cFilCT2 .And. ;
			...
			If lIsRedStor
				If __lMovEnt05
					...
					EndIf
				EndIf			
				U_CtbGrvRC(cMoeda,tpSaldo,"2",cContaG,cNormal,CTS->CTS_IDENT)
			Else	
				U_CtbGrvRC(cMoeda,tpSaldo,"2",cContaG,cNormal)
			EndIF
